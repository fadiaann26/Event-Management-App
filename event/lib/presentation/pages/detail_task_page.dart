...
import 'package:event/common/app_info.dart';
import 'package:event/common/enums.dart';
import 'package:event/common/urls.dart';
import 'package:event/common/utils.dart';
import 'package:event/data/models/task.dart';
import 'package:event/data/source/task_source.dart';
...
import 'package:event/presentation/widgets/app_button.dart';
import 'package:event/presentation/widgets/failed_ui.dart';
...
class _DetailTaskPageState extends State<DetailTaskPage> {
  ...
  submitTask() {
    TaskSource.submit(widget.id, attachment.value).then((success) {
      if (success) {
        AppInfo.success(context, "Success Submit Laporan");
        refresh();
      } else {
        AppInfo.failed(context, "Failed Submit Laporan");
      }
    });
  }
  ...
  rejectTask(String reason) {
    TaskSource.reject(widget.id, reason).then((success) {
      if (success) {
        AppInfo.success(context, "Success Reject Laporan");
        refresh();
      } else {
        AppInfo.failed(context, "Failed Reject Laporan");
      }
    });
  }
  ...
  fixTaskToQueue() {
    ...
    TaskSource.fixToQueue(widget.id, revision).then((success) {
      if (success) {
        AppInfo.success(context, "Success Fix Laporan to Queue");
        refresh();
      } else {
        AppInfo.failed(context, "Failed Fix Laporan to Queue");
      }
    });
  }
  ...
  approveTask() {
    TaskSource.approve(widget.id).then((success) {
      if (success) {
        AppInfo.success(context, "Success Approve Laporan");
        refresh();
      } else {
        AppInfo.failed(context, "Failed Approve Laporan");
      }
    });
  }
  ...
  deleteTask() {
    TaskSource.delete(widget.id).then((success) {
      if (success) {
        AppInfo.success(context, "Success Delete Laporan");
        Navigator.pop(context);
      } else {
        AppInfo.failed(context, "Failed Delete Laporan");
      }
    });
  }
      ...
      body: BlocBuilder<DetailTaskCubit, DetailTaskState>(
        builder: (context, state) {
          ...
          if (state.requestStatus == RequestStatus.failed) {
            return const FailedUI(message: "Laporan not Found");
          }
